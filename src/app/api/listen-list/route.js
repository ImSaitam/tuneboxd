...
import { albumService, watchlistService } from "../../../lib/database-adapter.js";
...
// POST: Añadir álbum a la lista de escucha
export async function POST(request) {
  try {
    ...
    // Verificar si ya está en la lista de escucha
    const isAlreadyInList = await watchlistService.isInWatchlist(decoded.userId, albumRecord.id);
    if (isAlreadyInList) {
      return Response.json(
    ...
    console.error('Error añadiendo álbum a la lista de escucha:', error);
    return Response.json(
...
// GET: Obtener la lista de escucha del usuario
export async function GET(request) {
  try {
    ...
    // Obtener lista de escucha
    const listenList = await watchlistService.getUserWatchlist(decoded.userId, limit, offset);
    ...
    console.error('Error obteniendo lista de escucha:', error);
    return Response.json(
      { success: false, message: 'Error interno del servidor' },
      { status: 500 }
    );
...
// DELETE: Remover álbum de la lista de escucha
export async function DELETE(request) {
  try {
    ...
    console.error('Error removiendo álbum de la lista de escucha:', error);
    return Response.json(
      { success: false, message: 'Error interno del servidor' },
      { status: 500 }
    );
