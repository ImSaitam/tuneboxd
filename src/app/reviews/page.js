      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-theme-primary">Reseñas de la Comunidad</h1>
              <p className="text-theme-secondary mt-1">Descubre qué opina la comunidad sobre los últimos álbumes</p>
            </div>
          </div>
          ...
          {isAuthenticated && (
            <div className="text-right">
              ...
              <p className="text-theme-secondary text-sm">¿Ya escribiste tu reseña hoy?</p>
            </div>
          )}
        </div>

        {/* Stats Banner */}
        <div className="bg-theme-card backdrop-blur-md border border-theme-border rounded-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div>
              <div className="text-3xl font-bold text-theme-primary">{reviews.length}</div>
              <div className="text-theme-secondary">Reseñas Recientes</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-yellow-400">
                {reviews.length > 0 ? (reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length).toFixed(1) : '0'}
              </div>
              <div className="text-theme-secondary">Rating Promedio</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-theme-accent">
                {new Set(reviews.map(r => r.album_id)).size}
              </div>
              <div className="text-theme-secondary">Álbumes Únicos</div>
            </div>
          </div>
        </div>
        ...
        {error ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-theme-primary mb-2">No hay reseñas aún</h3>
            <p className="text-theme-secondary mb-6">¡Sé el primero en escribir una reseña!</p>
            {isAuthenticated ? (
              <Link 
                href="/"
...
function ReviewCard({ review, onAlbumClick, onArtistClick }) {
  ...
  return (
    <div className="bg-theme-card backdrop-blur-md border border-theme-border rounded-lg p-6 hover:bg-theme-hover transition-all duration-300">
      <div className="flex gap-4">
        {/* Album Cover */}
        <div 
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1">
                <User size={16} className="text-theme-muted flex-shrink-0" />
                <span className="text-theme-primary font-medium">{review.username}</span>
                <span className="text-theme-muted">•</span>
                <Calendar size={16} className="text-theme-muted flex-shrink-0" />
                <span className="text-theme-secondary text-sm">{formatDate(review.created_at)}</span>
              </div>
              
              <div>
                <h3 
                  className="text-lg font-bold text-theme-primary truncate cursor-pointer hover:text-theme-accent transition-colors"
                  onClick={() => onAlbumClick(review)}
                >
                  {review.album_name}
                </h3>
                <p 
                  className="text-theme-secondary text-sm hover:text-theme-accent transition-colors cursor-pointer"
          ...
          {/* Review Title */}
          {review.title && (
            <h4 className="text-theme-primary font-semibold mb-2 text-lg">{review.title}</h4>
          )}

          {/* Review Content */}
          {review.content && (
            <p className="text-theme-secondary leading-relaxed mb-3 line-clamp-3">
              {review.content}
            </p>
          )}
          ...
          <div className="flex items-center gap-4 pt-2">
            <button
              onClick={() => onAlbumClick(review)}
              className="text-theme-accent hover:text-theme-primary transition-colors text-sm font-medium"
            >
              Ver Álbum
            </button>
