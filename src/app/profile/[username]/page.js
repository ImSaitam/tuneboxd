...
const UserProfilePage = () => {
  ...
  const { alert, confirm, success, error: showError } = useGlobalModal();
  const { success: showSuccess, error: showNotificationError } = useNotifications();
  ...
  const handleDeleteReview = async (reviewId) => {
    const confirmed = await confirm('¿Estás seguro de que quieres eliminar esta reseña? Esta acción no se puede deshacer.'); if (!confirmed) {
      return;
    }
