...
export const sendVerificationEmail = async (email, username, token) => {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.FROM_EMAIL || 'TuneBoxd <onboarding@resend.dev>',
      to: [email],
...
export const sendPasswordResetEmail = async (email, username, resetToken) => {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.FROM_EMAIL || 'TuneBoxd <onboarding@resend.dev>',
      to: [email],
      subject: '🔐 Restablece tu contraseña - Tuneboxd',
      html: `
        <!DOCTYPE html>
        <html lang="es">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Restablecer Contraseña - Tuneboxd</title>
        </head>
        <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0a0a0a; color: #ffffff; margin: 0; padding: 0;">
          <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
            <div style="background: linear-gradient(135deg, #1e1e1e 0%, #2a2a2a 100%); border-radius: 16px; padding: 40px; border: 1px solid #333;">
              ...
              <!-- Header -->
              <div style="text-align: center; margin-bottom: 40px;">
                <h1 style="color: #f59e0b; font-size: 32px; margin: 0; font-weight: bold;">🔐 Tuneboxd</h1>
                <p style="color: #9ca3af; margin: 10px 0 0 0; font-size: 16px;">Restablecimiento de contraseña</p>
              </div>
