...
export default function ListDetailPage() {
  ...
  const handleDeleteComment = async (commentId) => {
    const confirmed = await confirm('¿Eliminar este comentario?'); if (!confirmed) return;
  ...
  const handleRemoveAlbum = async (albumId) => {
    const confirmed = await confirm('¿Quieres remover este álbum de la lista?'); if (!confirmed) {
      return;
    }
